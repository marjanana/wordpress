...
class formsModelCfs extends modelCfs {
	...
	/**
	 * Copied from SocialSharing_Form_Module::getSearchString() as it is protected there
	 */
	/*protected function _socShareGetSearchString($id) {
        // Need to be a string
        $id = (string)$id;

        // Form ID saved in the database with the other serialized settings.
        // So we need to serialize value.
        $serialized = serialize(array('forms_id' => $id));
        // Take part like s:8:"forms_id";s:2:"54";
        $search = substr($serialized, 5, strlen($serialized) - 6);

        return '%' . $search . '%';
    }
	private function _updateSocSharingProject($projId, $formsId) {
		if(class_exists('SupsysticSocialSharing')) {
			...
			if(isset($supsysticSocialSharing) && !empty($supsysticSocialSharing) && method_exists($supsysticSocialSharing, 'getEnvironment')) {
				...
				$socShareFormMod = $supsysticSocialSharing->getEnvironment()->getModule('Form');
				if(!empty($socShareProjMod) && !empty($socShareFormMod)) {
					try {
						// Clear all prev. projects used this Form
						$searchPrjcts = $this->_socShareGetSearchString($formsId);
						$projects = $socShareProjMod->getModelsFactory()->get('projects');
						$project = $projects->searchByFormId($searchPrjcts);
						if(!empty($project)) {
							$project->settings['forms_id'] = 0;
						...
						if(!empty($projId)) {
							...
							if(!empty($socShareProj)) {
								$socShareProj->settings['where_to_show'] = 'forms';
								$socShareProj->settings['forms_id'] = $formsId;
